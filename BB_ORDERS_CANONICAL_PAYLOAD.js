...
for (const item of $input.all()) {
  ...
  out.push({ json: { ...r, items_json, items_text, items_count: items_json.length, total_quantity: items_json.reduce((sum, x) => sum + asNumber(x.quantity, 1), 0), packer_copy_text: first(r.packer_copy_text, items_text) ?? null, source_system: r.source_system ?? "front_house" } });
}
return out;
